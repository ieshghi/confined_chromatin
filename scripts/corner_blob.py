...
rho_init = np.exp(-((X-0.3)**2 + Y**2 + Z**2)/0.04)
...
rho_hist = plotting_utils.density_movie(wb,sh,r,simpars,rho_init,phi0,zers,undersamp = 1,name = 'density_mov_offcenter',minmax = [np.min(rho_init),np.max(rho_init)],pool = p)
